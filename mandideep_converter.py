...
def main():
    st.title("Mandideep Excel Converter")

    upload_file = st.file_uploader(label="Select the file", accept_multiple_files=False, key="mandideep")
    if upload_file is not None:
        ...
        fourth_half = dataframe[index[2] + 1: index[3] - 3].reset_index().drop(["index"], axis=1)
        fifth_half = dataframe[index[3] + 1:].reset_index().drop(["index"], axis=1)
        ...
        fifth_half = fifth_half.rename(columns=fifth_half.iloc[0]).drop(["From Date", "To Date", "nan"], axis=1).tail(
            fifth_half.shape[0] - 1)

        final_dataframe = pa.concat([first_half, second_half, third_half, fourth_half, fifth_half],
